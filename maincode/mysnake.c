...
void GAME_Handle(struct SNAKE* snake, struct FOOD* food, struct BARRIER* barrier){
 ...
 //success
 if(snake->length >= SUCCESS_VALUE){
    Success_Draw();
    Credit_Process(snake);
    Credit_Screen();
    msleep(3000);
    Success_Screen();
    g_start = 0;       //init the snake
...
void Success_Screen(){
	...
	//show credit on the screen
	for(i=0; i<37; i++){
		sys_fillrect(info, successimage[i]);
	}
}
...
//draw the success of the "SUCCESS"
void Success_Draw(){
	int i;
	for(i=0; i<37; i++){
		successimage[i]=kmalloc(sizeof(struct fb_fillrect), GFP_KERNEL);
		successimage[i]->color = CYG_FB_DEFAULT_PALETTE_WHITE;
		successimage[i]->rop = ROP_COPY;
	}
	//draw all hypen
	for(i=0;i<21; i++){
		if(i==8 || i==9 || i==10 || i==1){
			successimage[i]->color = CYG_FB_DEFAULT_PALETTE_BLACK;
		}
		successimage[i]->dx = 40+60*(i%7);      		//hypen_dx
		successimage[i]->dy = 80 + (i/7) * 45;  		//hypen_dy
		successimage[i]->width = 40;
		successimage[i]->height = 5;
	}

	//left vertical above
	for(i=21; i<28; i++){
		successimage[i]->dx = 35+60*(i-21);      		//hypen_dx
		successimage[i]->dy = 85;  						//hypen_dy
		successimage[i]->width = 5;
		successimage[i]->height = 40;
	}

	//left vertical below
	for(i=28; i<32; i++){
		successimage[i]->dx = 95 + 60*(i-28);      		//hypen_dx
		successimage[i]->dy = 130;  						//hypen_dy
		successimage[i]->width = 5;
		successimage[i]->height = 40;
	}
	...
	for(i=32; i<34; i++){
		successimage[i]->dx = 80 + 60*(i-32);      		//hypen_dx
		successimage[i]->dy = 130;  		//hypen_dy
		successimage[i]->width = 5;
		successimage[i]->height = 40;
	}
	...
	for(i=34; i<36; i++){
		successimage[i]->dx = 380 + 60*(i-34);      		//hypen_dx
		successimage[i]->dy = 130;  		//hypen_dy
		successimage[i]->width = 5;
		successimage[i]->height = 40;
	}

	successimage[36]->dx = 140;      		//hypen_dx
	successimage[36]->dy = 85;  		//hypen_dy
	successimage[36]->width = 5;
	successimage[36]->height = 40;	
}
